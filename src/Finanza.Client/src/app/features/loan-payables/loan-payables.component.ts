...
import { LoanPayableService, LoanPayableRequest } from '../../core/services/loan-payable.service';
import { DashboardTabsComponent } from '../../shared/dashboard-tabs/dashboard-tabs.component';
import { LoanPayable, LoanPayableInstallment, LoanPayableSummary } from '../../core/models/loan-payable.model';
import { ConfirmDialogComponent } from '../../shared/components/confirm-dialog/confirm-dialog.component';
import { CurrencyMaskDirective } from '../../core/directives/currency-mask.directive';

@Component({
  selector: 'app-loan-payables',
  standalone: true,
  imports: [
  ...
  templateUrl: './loan-payables.component.html',
  styleUrl:    './loan-payables.component.scss',
})
export class LoanPayablesComponent implements OnInit {
  private readonly service  = inject(LoanPayableService);
  ...
  loading  = signal(true);
  saving   = signal(false);
  loans    = signal<LoanPayable[]>([]);
  summary  = signal<LoanPayableSummary | null>(null);
  editing  = signal<LoanPayable | null>(null);
  ...
  openAdd(): void {
    ...
    this.form.reset({ creditorName: '', totalAmount: null, startDate: '', dueDate: '', notes: '', installmentCount: 1 });
    this.showForm.set(true);
  }

  openEdit(loan: LoanPayable): void {
  ...
  payInstallment(installment: LoanPayableInstallment): void {
  ...
  unpayInstallment(installment: LoanPayableInstallment): void {
  ...
  delete(loan: LoanPayable): void {
    const ref = this.dialog.open(ConfirmDialogComponent, {
      width: '400px',
      data: { title: 'Excluir empréstimo', message: `Excluir empréstimo de <strong>${loan.creditorName}</strong>? Esta ação remove todas as parcelas.` },
    });
  ...
  progressRate(loan: LoanPayable): number {
